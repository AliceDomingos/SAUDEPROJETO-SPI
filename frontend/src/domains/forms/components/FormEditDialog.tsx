...
export default function FormEditDialog({ formId, onClose, onSaved }: Props) {
  const isOpen = formId !== null;
  ...
  useEffect(() => {
    ...
    getGrupos().then((lista) => {
      setGrupos(lista);
      if (!isAdmin && lista.length === 1) setGroupId(lista[0].id);
    }).catch(() => {});
  ...
  return (
    <Dialog
      ...
      description="Alterar dados do formulário"
      onClose={onClose}
    ...
    >
      {loading ? (
        ...
        <form id="form-edit-form" onSubmit={handleSubmit} className="space-y-5 max-h-[60vh] overflow-y-auto pr-1">
          {error && (
